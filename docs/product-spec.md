...
1. Open UnseenLab (no account). The homepage asks: *"What topic do you need help with?"*
2. Type a topic (e.g., "chain reaction") — a supported topic routes into the Nuclear Chain Reaction lab; an unsupported topic gets a friendly explanation plus a pointer to the available lab. Alternatively use the "Start this lab" link on the "Available lab" card.
...
12. Submit an updated prediction (post-trial confidence) — this unlocks the next trial.
13. Run another trial (a second, third, or later — no reload needed); every trial is appended to the recorded evidence and the loop repeats from step 6.
14. Open Adaptation Replay to review the whole multi-trial journey.
15. Optional: research mode — pre/post questions, mental effort, free-text feedback, JSON export, clear local session.
...
The former "Feedback timing" dimension is intentionally gone: its control was removed because it was never wired to any behavior (a dead control breaks trust). If feedback timing is ever implemented, it returns as a dimension with a real effect.
...
- Landing page with topic-input hero ("What topic do you need help with?"), supported/unsupported topic routing, "How it works", "Available lab" card with a "Start this lab" primary action, "Future labs" list (non-interactive), accessibility section, and the conceptual-safety disclaimer in the footer.
...
- Prediction required before every run; confidence 1–5 before and after; an updated prediction gates each new trial (repeatable multi-trial loop, no reload).
- Deterministic seeded simulation with safety caps (population ceiling, step limit, clamping, no negatives).
- Adaptation provider interface with a working deterministic offline implementation and an optional structured LLM provider behind a server bridge (`POST /api/adapt`); every proposal carries evidence IDs.
...
- Adaptation Replay: initial prediction → variables changed → outcome → interaction pattern → possible conceptual friction → adaptation → decision → updated prediction → counterfactual → evidence of changed understanding, across all trials in the session.
...
1. Open UnseenLab. 2. Type "chain reaction" in the topic input (or "Start this lab"); land in the lab. 3. Choose animation-first, low text density, interactive mode. 4. Read the goal. 5. Enter prediction: "The reaction gets slightly faster." 6. Set absorber position lower. 7. Run the seeded trial. 8. Watch the neutron population accelerate nonlinearly to the safety ceiling. 9. Show the graph. 10. Display prediction vs. observed result and possible `LINEAR_VS_NONLINEAR_GROWTH` friction. 11. Offer: freeze all variables except absorber position · slow animation · compare two trials. 12. Accept. 13. Run the Counterfactual Microscope (absorber position). 14. Show side-by-side trials. 15. Submit updated prediction ("The neutron population may grow nonlinearly.") — this unlocks the next trial. 16. Change one variable and run a second trial; the loop repeats, trials are appended, and the replay now shows both trials and the reasoning change. 17. Open Adaptation Replay to review the multi-trial journey. No step depends on an external model API — the optional hosted path only adds an "AI interpretation" label when it is enabled and healthy; otherwise the same flow runs on the deterministic rules with an "Offline rules" badge.
