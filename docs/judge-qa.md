...
**Honest answer:** The AI layer (1) interprets learner predictions (structured choices or conservative keyword classification of free text), (2) maps behavior and outcomes onto a bounded misconception taxonomy (five concepts), and (3) selects targeted, explainable pedagogical interventions (freeze variables, slow animation, show graph, compare trials, causal view, reduce density, ask again) — each with evidence IDs and a plain-language reason. It proposes; the learner disposes. Without it, the app is still a complete, deterministic virtual lab: prediction → seeded simulation → animation → graph/equation/causal/plain-language views → counterfactual comparison → replay. Everything scientific works offline with no API key. The current provider is deterministic; a future structured LLM provider can slot in behind the same `AdaptationProvider` interface. There is no generic unrestricted chat endpoint.

**Product evidence:** `src/adaptation/` (interface + deterministic provider + taxonomy), isolated scientific core in `src/simulation/`, tests for both layers.

**Missing evidence:** the structured LLM provider implementation.
...
**Honest answer:** The learner controls everything. All fifteen accessibility/display settings are explicit controls with readable values, persisted locally. Adaptations are offers, not actions: each card states what was observed, what would change, and why it may help, with Accept / Reject / Modify buttons; a rejected proposal type is never re-offered; every preference can be manually overridden. There are no clinical labels, no diagnosis-based presets, and the product never says "the system diagnosed a misconception" — it speaks of "possible conceptual friction". No timer, no forced audio, no flashing, no account.
