#!/usr/bin/env node
/**
 * Holdout V3 runner — hosted demonstration-spec model (FROZEN 2026-08-06).
 *
 * Executes the frozen holdout manifest
 * (scripts/holdout-v3-manifest-2026-08-06.mjs, holdout-2026-08-06-v3) against
 * the REAL hosted generation endpoint (POST /api/demonstrations/generate) —
 * the same harness entry the v1/v2 runners and the Gate-2 script use. The
 ...
 *     docs/holdout-2026-08-06-v3.md). Any manifest edit after the freeze is
 ...
 *
 * SCORING (frozen, verbatim — see docs/holdout-2026-08-06-v3.md):
 ...
 *
 * Usage:
 *   node scripts/holdout-v3-runner-2026-08-06.mjs [--base http://localhost:3200] [--rate 20]
 ...
 *
 * Output:
 *   - docs/holdout-v3-results-2026-08-06.json — full per-prompt results +
 *     metrics (RAW / REPAIRED / FALLBACK / FINAL splits)
 *   - printed frozen scorecard + PASS/FAIL gate summary
 */

import { HOLDOUT, META } from "./holdout-v3-manifest-2026-08-06.mjs";
...
/** SHA-256 of JSON.stringify(HOLDOUT) — recorded in docs/holdout-2026-08-06-v3.md. */
...
/**
 * Frozen ENGINE_CONTROL_CATALOG subset for the four engines used by v3 engine
...
const RESULTS_PATH = join(
  ...
  "..",
  "docs",
  "holdout-v3-results-2026-08-06.json",
);
...
// ---------------------------------------------------------------------------
// Structural validation of a returned spec (mirrors v1 / v2 / Gate-2)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Hosted endpoint call (identical harness to v1/v2 runners / Gate-2)
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  console.log(`HOLDOUT V3 RUN (FROZEN 2026-08-06) — base ${BASE} · ${HOLDOUT.length} prompts · batch ${BATCH}`);
  ...
  for (let i = 0; i < HOLDOUT.length; i += 1) {
    ...
    const ctrlNote =
      row.outcome === "spec" && entry.gold.kind === "engine"
        ? ` · controls[${(row.specControls ?? []).filter((c) => c && c.target && c.target.kind === "parameter").map((c) => c.target.ref).join(",") || "none"}] cov=${row.controlRelevance.coverage}`
        : "";
    console.log(
...
main().catch((err) => {
  console.error("holdout v3 run failed:", err);
  process.exit(1);
});
